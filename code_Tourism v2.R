...
opt_level <- "level_2"
setwd(paste0("C:/Users/vangelis spil/Desktop/Tourism_CHF/",opt_level))
...
# library(fpp3)
# Data <- as.data.frame(tourism)
# Data$Purpose <- NULL
# Data <- ddply(Data, .(State, Region, Quarter), colwise(sum))
# write.csv(Data, "Tourism_Data.csv", row.names = F)
Data <- read.csv("Tourism_Data.csv", stringsAsFactors =  F)
Data$StateOR <- Data$State ; Data$RegionOR <- Data$Region
...
for (i in 1:length(map)){
  temp <- Data[Data$State==map[i],]
  
  map2_r_t <- unique(temp$Region)
  map2_t <- as.character(c(1:length(map2_r_t)))
  map2_t[nchar(map2_t)==1] <- paste0("0",map2_t[nchar(map2_t)==1])
  
  map2 <- c(map2, map2_t)
  map2_r <- c(map2_r, map2_r_t)
}
for (i in 1:length(map2_r)){
  Data[Data$Region==map2_r[i],]$Region <- map2[i]
}
Data$Flag <- paste0(Data$State,Data$Region)
series_ids <- unique(Data$Flag)

DataFor <- Data[Data$Flag==series_ids[1],c("Quarter","Trips")]
colnames(DataFor)[2] <- series_ids[1] ; row.names(DataFor) <- NULL
for (i in 2:length(series_ids)){
  DataFor_t <- Data[Data$Flag==series_ids[i],c("Quarter","Trips")] 
  colnames(DataFor_t)[2] <- series_ids[i] ; row.names(DataFor_t) <- NULL
  DataFor <- cbind(DataFor, DataFor_t)
  DataFor <- DataFor[,-(ncol(DataFor)-1)]
}
DataFor$Quarter <- NULL
for (i in 1:length(series_ids)){
  DataFor[,i] <- ts(DataFor[,i], frequency = 4, start = c(1998,1))
}
rm(map,map2,map2_r,map_r,i,temp,DataFor_t,map2_r_t,map2_t,series_ids)

###################################################################################################
#Tourism data set
###################################################################################################
DataForHierarchy <- hts(ts(DataFor, frequency = 4, start = c(1998,1)), characters = c(1,2))
...
tslength <- 80#60 #Available observations
...
while ((origin+counter+fh) <= tslength){
  ...
  hts.data.scan.train <- hts(train_sample[,(ncol(train_sample)-b_series+1):ncol(train_sample)], characters = c(1,2))
  hts.data.scan.test <- hts(test_sample[,(ncol(test_sample)-b_series+1):ncol(test_sample), drop=FALSE], characters = c(1,2))
  ...
  for (model_id in 1:3){
    ...
    if (opt_level=="all_levels"){
      Min_Error_MASE <- (Errors[1,1]+mean(Errors[2:9,1])+mean(Errors[10:85,1]))/3 
      ...
      Min_Error_MASE <- mean(Errors[10:85,1])
    }
    ##################################################
...
##I excluded some other features below
...
save.image("TrainSample80.Rdata")


load("TrainSample80.Rdata")
...
counter <- 0
origin_id <- 60 #Starting origin (26weeks)
tslength_id <- 80 #Available observations
fh = 2 #Forecasting horizon considered
svmC = xgvC = RFc <- c()
...
while ((origin_id+counter+fh) <= tslength_id){
  ...
  ############ Optimization criterion ################
  if (opt_level=="all_levels"){
    Errors_test1$best <- which.min(c((Errors_test1[1,1]+mean(Errors_test1[2:9,1])+mean(Errors_test1[10:85,1]))/3,
                                     (Errors_test1[1,2]+mean(Errors_test1[2:9,2])+mean(Errors_test1[10:85,2]))/3,
                                     (Errors_test1[1,3]+mean(Errors_test1[2:9,3])+mean(Errors_test1[10:85,3]))/3))
  }else if (opt_level=="all_series"){
    ...
    Errors_test1$best <- which.min(c(mean(Errors_test1[10:85,1]),
                                     mean(Errors_test1[10:85,2]),
                                     mean(Errors_test1[10:85,3])))
  }
  ##################################################
  
  Errors_test1$counter <- counter+2
  Errors_test1$level <- c(1,rep(2,8),rep(3,76))
  ...
  set.seed(100)
  kernfit <- ksvm(as.matrix(dat[,-dim(dat)[2]]),dat$y, type = "C-svc", kernel = 'polydot', 
                  C = 100, scaled = TRUE)
